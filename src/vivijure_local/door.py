...
from . import i2v_ltx as _engine

SERVICE = "vivijure-local-12gb"   # /health service name + operator-facing labels
ENGINE = "ltx-video"              # engine label in the /health + selftest envelopes

# First-cold-render heads-up (weight download size + wait): printed once by the ready banner (announce).
WEIGHTS_NOTE = "the LTX-Video weights (~10GB, one time), so it takes several extra minutes. Later renders skip it."
