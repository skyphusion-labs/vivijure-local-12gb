"""The VRAM cap knob for the 12GB door (pure math).
...
real silicon at the 12GB budget (docs/proof/RESULTS.md), and every request is clamped to those tier
ceilings. (A scaffold-era pre-load VRAM estimator used to live in this module; it was never wired into
the server, so it was removed rather than keep advertising a safety check that never ran.)
"""
