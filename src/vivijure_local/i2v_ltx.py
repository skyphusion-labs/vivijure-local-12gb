...
def _step_callback(progress_cb, total: int):
    ...
    a /cancel actually aborts the denoise between steps (#87); swallowing it made cancel a silent no-op."""
    if progress_cb is None:
        return None

    from .core.jobs import Cancelled  # cooperative-cancel signal; MUST propagate so /cancel aborts denoise (#87)

    def on_step_end(pipe, step_index, timestep, callback_kwargs):
        try:
            progress_cb(step_index + 1, total)
        except Cancelled:
            raise  # cooperative cancel (#87): abort the denoise loop, never swallow the signal
        except Exception:
            pass
