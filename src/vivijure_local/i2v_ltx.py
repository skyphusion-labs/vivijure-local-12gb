...
# time -- a 12GB card cannot fit two i2v pipelines), so the honest optimisation is the opposite: keep the
# resident pipeline warm instead of re-reading the full weights (~30s + a full disk/VRAM reload) every
...
def _evict_pipe(cfg: I2VConfig, torch) -> None:
    ...
    -- the 12GB budget runs on ~1.2GB headroom (docs/proof/RESULTS.md), too thin to wait on the collector."""
...
def animate(shot_id: str, keyframe: Path, prompt: str, cfg: I2VConfig, out_path: Path, *, progress_cb=None) -> I2VResult:
    ...
    the FIRST job on a warm box pays the weights load; `progress_cb(step, total)` is wired best-effort
    through diffusers' callback hook. On a failed generate the pipeline is evicted and its VRAM freed
    (`_evict_pipe`) so a poisoned pipe never carries into the next job.
    ...
    pipe = _get_pipe(cfg, LTXImageToVideoPipeline, torch)
