#!/usr/bin/env python3
"""The PROOF GATE: a live LTX-Video i2v benchmark on real 16GB hardware.

Scripted + ready to FIRE the instant Conrad picks the card (his own 4060 Ti = $0 + the real target,
or a RunPod 16GB Ada = ~$1-5). DO NOT run it until he says go -- it needs a GPU + downloads weights
...
What it captures, per quality tier (draft / standard / final):
  - FIT:   peak VRAM (torch.cuda.max_memory_allocated) and whether it OOMs at the 16GB ceiling.
...
This intentionally does NOT touch R2 or the job server -- it drives the engine (i2v_ltx.animate)
...
from vivijure_local import i2v_ltx, vram  # noqa: E402
...
def main() -> int:
    ap = argparse.ArgumentParser(description="LTX-Video i2v 16GB benchmark (the proof gate)")
...
def _write_markdown(path: Path, report: dict) -> None:
    m = report["meta"]
    lines = [
        f"# LTX-Video i2v 16GB benchmark -- {m['host']}",
        "",
        f"- GPU: **{m['gpu']}**",
