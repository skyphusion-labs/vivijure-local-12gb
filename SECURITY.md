...
This is the **local-consumer render backend**: image-to-video (LTX-Video) that a self-hoster runs on
...
- HIGH -- untrusted-artifact-load: `transformers` remote code execution, the `diffusers`
  ...
  deserialization of untrusted data. This backend loads ONLY a PINNED model id (`Lightricks/LTX-Video`), never a
