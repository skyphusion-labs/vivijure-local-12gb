...
long-running server that does image-to-video on a SINGLE consumer GPU (the RTX 4060 Ti 16GB floor)
...
already own). This repo is the second door. **Private repo.** AGPL-3.0-only.

```
control plane --> local-gpu module (CF Worker) --/run--> CF tunnel --> THIS backend (LTX-Video, RTX 4060 Ti 16GB)
```
...
16GB consumer floor, the `local-gpu` path). The studio talks to THIS server with the SAME job lifecycle
it uses for RunPod (`/run` -> `/status/<id>` -> output), so the local-gpu module is a near-drop-in of the
cloud module. The tiers map to what a 16GB card can HONESTLY deliver: `final` is the card's ceiling,
...
- `docs/i2v-model-selection.md` -- why LTX-Video over CogVideoX / SVD / AnimateDiff for the 16GB floor.
...
16GB floor. Validated peak ~10.4GB (no OOM, ~6GB headroom): draft 38.6s/clip, standard 125.6s/clip.
...
- **A producer stage never ships a fake clip.** The generation body raises a clear error when the GPU
  runtime is absent rather than faking output; tiers advertise only what a 16GB card honestly delivers.
...
- The FIRST command in any op is the member's own login shell: `sudo -u <member> bash -lc '<ops>'`
  (loads their `$HOME`, their `~/dev/vivijure-local-backend` clone, their gh / R2 creds). Commits and
