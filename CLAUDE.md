...
- `.github/workflows/runner-snapshot.yml` -- pre-pulls the runtime base into a custom runner image for
  cache-warm publish builds (`local-12gb-bake-snap`, enterprise UI).
...
- **One server, serial registry.** `core/server.py` is a long-running process with an in-process job
  ...
  helpers are `core/contract.py` / `core/r2.py`; VRAM math is `core/vram.py`; the tier config is `config.py` (per-door); the docker-compose
  `ready` banner is `core/announce.py`. The shared surface (`r2` / `contract` / `jobs` / `vram` / `announce` / `server`) lives in the byte-identical `vivijure_local.core` package; `door.py` is the per-door identity + engine seam (see docs/architecture.md).
