# vivijure-local-12gb RUNTIME BASE image (the heavy, slow-changing layer set).
#
...
#
#   ghcr.io/skyphusion-labs/vivijure-local-12gb:runtime-t<N>   (a TAG in the same package)
#
# Same package as the release tags on purpose: FROM-inheritance dedups within one GHCR package. NO model
# weights are baked here (LTX pulls on the first render into a volume), so unlike vivijure-backend there
# is no seed image -- this base is a plain ~8GB pull.
#
