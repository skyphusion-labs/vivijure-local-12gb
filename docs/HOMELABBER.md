...
matrix. When you're done, run `./preflight.sh` (below) to confirm every piece is in place.
...
test, not just "is the package installed"), your GPU's VRAM, and free disk. Every failed check names the
step above that fixes it, and the script exits non-zero; **all green means you're ready** for the R2
step and `docker compose up`.
