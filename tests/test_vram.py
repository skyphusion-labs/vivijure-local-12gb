...
def test_all_three_tiers_are_estimated_to_fit_the_12gb_floor():
    for tier in (QualityTier.DRAFT, QualityTier.STANDARD, QualityTier.FINAL):
        est = vram.estimate(_cfg(tier))
        assert est.fits, f"{tier} predicted not to fit 12GB: {est}"
...
def test_strongest_offload_picks_the_weakest_that_fits():
    # The base LTX i2v (~14GB fully resident) is too tight for 12GB without offload, so the budgeter
...
def test_a_24gb_card_has_more_headroom_than_the_12gb_floor():
    cfg = _cfg(QualityTier.FINAL)
    floor = vram.estimate(cfg, card_gb=12.0)
